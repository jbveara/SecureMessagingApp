...
socket.send_multipart([b"REGISTER", bytes(username,'utf-8'), user.SerializeToString()])
...
while(True):
    ...
    # if input on stdin -- process user commands
        ...
        # SEND command is sent as a three parts ZMQ message, as "SEND destination message"
            ...
            socket.send_multipart([bytes(cmd[0],'utf-8'), bytes(cmd[1],'utf-8'), bytes(cmd[2],'utf-8')])
