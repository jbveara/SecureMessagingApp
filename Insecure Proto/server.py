...
while(True):
    ...
    print("Received [%s]" % (message[0]))

    if len(message) == 2:
    	if message[1]== b'LIST':
            # If first seeing this identity sent back ERR message requesting a REGISTER
            if ident not in logged_ident: socket.send_multipart([ident, b'ERR', b'You need to register first.'])
            else:
                print("List request from user %s" %(logged_ident[ident]))
                socket.send_multipart([ident, b'LIST', base64.b64encode(str(logged_users).encode('utf-8'))])
    ...
    if len(message) == 4:
        if message[1] == b'REGISTER':
            ...
            socket.send_multipart([ident, b"REGISTER", b'Welcome %s!' % bytes(user.name, 'utf-8')])
